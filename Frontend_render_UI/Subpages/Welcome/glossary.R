...
img_get <- function(abbr) switch(
  ...
  los = "statistical_methods_images/biomolecule_data.jpg",
  ppp = "statistical_methods_images/biomolecule_data.jpg",
  rip = "statistical_methods_images/biomolecule_data.jpg",
  ppp_rip = "statistical_methods_images/biomolecule_data.jpg",
...
text_get <- function(abbr) switch(
  ...
  rrollup = paste0("A reference-based summarization of peptide data was performed to estimate protein abundances. For a peptides assigned to a protein, this method utilized",
    " the abundance of the peptide with the most observations as a 'reference' to scale all other",
    " peptide abundances mapped to the same protein. Where a tie in the number of observations occurs, the peptide with the greatest median abundance was",
    " selected as the reference. After scaling, either the mean or median (user-selected) of assigned peptides in each sample was used as the final protein quantification."),
  qrollup = paste0("A quantile-based summarization of peptide data was performed to estimate protein abundances. For all peptides assigned to a protein, this method only",
    " kept peptides whose mean was a above a user-specified threshold for all peptides assigned to that protein, essentially keeping on average the most abundant ",
    "peptides per protein. After filtering, either the mean or median (user-selected) of the remaining peptides aundances was used as the final protein quantification."),
  zrollup = paste0(
    "A z-score based summarization of peptide data was performed to estimate protein abundances. For a peptides assigned to a protein, this method calulated ",
    " the median and standard deviation of all peptide abundances mapped to the same protein. The peptides medians were substracted",
    " from their respective peptide abundances in each sample, then divided by the respective standard deviation. After this scaling, either the mean or median (user-selected) of assigned peptides in each sample was used as the final protein quantification."),
  spans = paste0("An automated process was used to assess the best normalization method for proteomic data. This method iterated through several possible normalizations",
    ...
    "experimental groups), the normalization was assessed for conservation of differentially expressed biomolecules before and after normalization. Normalizations with the best results recieved higher numeric scores."),
  rmd_filter = paste0("A robust Mahalanobis distance (rMd) of non-parametric data properties was computed to identify possible outlier samples. A robust principal components",
  ...
  median_normalization = paste0("Median normalization was performed to reduce variation unrelated to the experimental conditions of interest. The median of all peptides in the",
    " specified subset were caculated for each sample, then used to scale all values for each respective sample."),
  mean_normalization = paste0("Mean normalization was performed to reduce variation unrelated to the experimental conditions of interest. The median of all peptides in the",
                              " specified subset were caculated for each sample, then used to scale all values for each respective sample."),
  zscore = paste0("Z-score transformation was performed to reduce variation unrelated to the experimental conditions of interest. Each feature was scaled by subtracting",
  ...
  mad = paste0("Median absolute deviation (MAD) transformation was performed to reduce variation unrelated to the experimental conditions of interest. Each feature was scaled by ",
  ...
  all_b = "All biomolecules were used in metrics to scale/transform the data.",
  complete_b = "All biomolecules without missing observations were used in metrics to scale/transform the data.",
  los = "Biomolecules associated with the top L proportion of highest absolute abundance, where L was between 0 and 1, were used in metrics to scale/transform the data.",
  ppp = "Biomolecules present in at or above a user-specified porpotion of samples were used in metrics to scale/transform the data.",
  rip = "All biomolecules without missing observations that had a Kruskal-Wallis on group membership p-value greater than a user-defined threshold were used in metrics to scale/transform the data.",
  ppp_rip = "Biomolecules present in at or above a user-specified porpotion of samples that had a Kruskal-Wallis on group membership p-value greater than a user-defined threshold were used in metrics to scale/transform the data.",
  backtransform = "Normalized values were backtransformed such that the normalized values were on a similar scale as the orginal unnormalized values. All values were backtransformed by a global metric taken across all samples prior to normalization depending on the type of normalization."
)
