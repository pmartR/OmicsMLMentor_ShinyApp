...
output$e_data_load_UI <- renderUI({
   ...
   req(AWS || input$upload_model_done > 0 || input$load_example > 0)
...
output$f_data_load_UI <- renderUI({
   ...
   req(AWS || input$specify_edata_done > 0 || input$load_example > 0)
...
observeEvent(reactive_dataholder$f_data,{
   if(is.null(reactive_dataholder$f_data)){
      disable("upload_fdata_done")
   }
}, ignoreNULL = FALSE)

output$e_meta_load_UI <- renderUI({

   req((input$upload_fdata_done > 0 && input$use_fdata == "No") ||
...
observeEvent(reactive_dataholder$e_meta,{
   if(is.null(reactive_dataholder$e_meta)){
      disable("upload_emeta_done")
   }
}, ignoreNULL = FALSE)
