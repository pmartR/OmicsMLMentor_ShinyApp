  ...
  observeEvent(input[[paste0(label, "_file")]], ignoreInit = TRUE, {
    ...
    if(label %in% data_select_val){
      ...
      if(label == "model"){
        ...
        responses <- unique(model$model$pre$mold$outcomes[[1]])
        og_train_size <- model$model$pre$mold$blueprint$recipe$tr_info[[1]]
        n_predictors <- ncol(model$model$pre$mold$blueprint$ptypes$predictors)


        hyperparams <- model$model$fit$actions$model$spec$args
        ...
        performance <- attributes(model$model)$prediction_train %>%
          data.frame() %>%
