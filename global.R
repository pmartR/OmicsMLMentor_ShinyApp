...
file_loads <- c(
  ...
  list.files("./Static_UI", recursive = T, full.names = T)
)
...
TABS_CHANGE_STATE <- c(
  "Upload", "Reference", "Groups",
  "Filter", "Normalization", "Pepstats",
  "Roll-up", "EDA", "Statistics",
...
ttext <- list(
  ...
  BAD_COMPARISONS = "The following comparisons are confounded in one or more datasets, please remove one or more of these columns from your grouping selections: %s",
  ...
  CV_THRESH_TOO_LOW = "Maximum CV must be between 1 and the maximum data CV: %s",
  DATA_IMPORTED = "Your data has been uploaded from a SLOPE project, continue by completing the remainder of the %s tab.",
  DOWNLOAD_OPTS_DISABLED = "Currently selected parameters are the same as the ones currently stored for this plot",
  GTEST_TOO_FEW = "All groups must have at least 3 nonmissing observations for at least one biomolecule to perform the G-test, the following groups did not: %s",
  ...
  IMDANOVA_VALUES_OUT_OF_RANGE = "The specified minimum number of observations per group must be less than or equal to the minimum group size: %s <br/> Minimum g-test must be >= 3.  Minimum ANOVA must be >=2.",
  ...
  RMDFILT_INSUF_EDATA = "Must have at least 100 biomolecules to apply a rMd filter, current data has %s",
  RMDFILT_NO_VARIANCE = "The following metrics did not vary across your sample: %s. Please re-add the filter without this metric to continue.",
  ...
  TOO_FEW_NONSINGLETONS = "The following main effects or interaction effects had less than two non-singleton groups: %s",
