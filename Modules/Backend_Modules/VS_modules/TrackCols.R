  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$picker_inputs <- renderUI({
        ...
        track_f_data <- colnames(file_reactive$f_data_file)
        track_e_meta <- colnames(file_reactive$e_meta_file)
        ...
        pickers <- map2(list(track_e_data, track_f_data, track_e_meta),
                        c("e_data", "f_data", "e_meta"), function(x, lab){
                          if(length(x) != 0){
                            ...
                            selected <- isolate(
                              if(!is.null(input[[paste0(lab, "_track")]])){
                                c(file_reactive[[paste0(lab, "_cname")]], input[[paste0(lab, "_track")]])
                              } else x
                            )
